...
create or replace NONEDITIONABLE PROCEDURE cmp_prc (prod IN VARCHAR2, dev IN VARCHAR2)
AUTHID CURRENT_USER
AS
    prod_name        VARCHAR2(100) := UPPER(prod);
    dev_name         VARCHAR2(100) := UPPER(dev);
    v_prod_proc_name VARCHAR2(100);
    v_dev_proc_name  VARCHAR2(100);
    v_prod_text      CLOB;
    v_dev_text       CLOB;
    v_prod_lines     SYS.ODCIVARCHAR2LIST;
    v_dev_lines      SYS.ODCIVARCHAR2LIST;
BEGIN
    -- �������� �� ���������� � ����� prod.
    FOR prod_proc IN (SELECT object_name
                      FROM all_objects
                      WHERE object_type = 'PROCEDURE' AND owner = prod_name)
    LOOP
        v_prod_proc_name := prod_proc.object_name;

        -- ��������� ������ ��������� �� ����� prod.
        SELECT DBMS_METADATA.GET_DDL('PROCEDURE', v_prod_proc_name, prod_name) INTO v_prod_text FROM DUAL;

        -- ���������� ����� �� ������ ��������� � ����� prod.
        SELECT REGEXP_SUBSTR(v_prod_text, '(.*?);', 1, LEVEL, NULL, 1) BULK COLLECT INTO v_prod_lines
        FROM DUAL
        CONNECT BY LEVEL <= REGEXP_COUNT(v_prod_text, '(.*?);');

        -- �������� �� ���������� � ����� �� ������ � ����� dev.
        FOR dev_proc IN (SELECT object_name
                         FROM all_objects
                         WHERE object_type = 'PROCEDURE' AND owner = dev_name AND object_name = v_prod_proc_name)
        LOOP
            v_dev_proc_name := dev_proc.object_name;

            -- ��������� ������ ��������� �� ����� dev.
            SELECT DBMS_METADATA.GET_DDL('PROCEDURE', v_dev_proc_name, dev_name) INTO v_dev_text FROM DUAL;

            -- ���������� ����� �� ������ ��������� � ����� dev.
            SELECT REGEXP_SUBSTR(v_dev_text, '(.*?);', 1, LEVEL, NULL, 1) BULK COLLECT INTO v_dev_lines
            FROM DUAL
            CONNECT BY LEVEL <= REGEXP_COUNT(v_dev_text, '(.*?);');

            -- ��������� ����� �������� ���������.
            FOR i IN 1 .. GREATEST(v_prod_lines.COUNT, v_dev_lines.COUNT)
            LOOP
                IF i <= v_prod_lines.COUNT AND i <= v_dev_lines.COUNT THEN
                    IF v_prod_lines(i) <> v_dev_lines(i) THEN
                        DBMS_OUTPUT.PUT_LINE('Difference found between ' || prod_name || '.' || v_prod_proc_name || ' and ' || dev_name || '.' || v_dev_proc_name);
                        DBMS_OUTPUT.PUT_LINE(prod_name || '.' || v_prod_proc_name || ': ' || v_prod_lines(i));
                        DBMS_OUTPUT.PUT_LINE(dev_name || '.' || v_dev_proc_name || ': ' || v_dev_lines(i));
                        --dmbs_output.put_line('CREATE OR REPLACE PROCEDURE ' || v_dev_proc_name || );
                        dbms_output.put_line('CREATE OR REPLACE PROCEDURE ' || v_prod_proc_name || '(salary in number, percent in number) as');
                        dbms_output.put_line('BEGIN');
                        FOR j in 1 .. GREATEST(v_prod_lines.COUNT, v_dev_lines.COUNT)
                        LOOP
                            dbms_output.put_line(v_prod_lines(j));
                        END LOOP;
                    END IF;
                ELSIF i <= v_prod_lines.COUNT THEN
                    DBMS_OUTPUT.PUT_LINE(prod_name || '.' || v_prod_proc_name || ': ' || v_prod_lines(i));
                ELSE
                    DBMS_OUTPUT.PUT_LINE(dev_name || '.' || v_dev_proc_name || ': ' || v_dev_lines(i));
                END IF;
